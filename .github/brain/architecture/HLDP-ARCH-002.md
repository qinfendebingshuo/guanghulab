...
└──────────────────────────────────────────────────────────┘
                             ...
                             ▼
┌──────────────────────────────────────────────────────────┐
│ 人格体核心大脑 MP · Qwen3-1.7B 蒸馏 + 微调（每人格一个）    │
...
### Phase A · M0 母体（8B 世界观底色）
- 模型: Qwen3-8B-Base
- 语料: 全量光湖语料（GPT + Notion + GitHub 自然语言 ~5-8 亿 token）
- 方法: CPT（continued pretraining）+ 轻量 SFT
- **不做**: 复杂指令对齐 / 长 CoT 训练（那是商业 API 的活）
- 产出: M0-v1 · 用作世界观底色 + MP 蒸馏教师

### Phase B · 不再训练独立的 8B 代码模型 MC（方案修正）
> 修正说明：原方案中的 MC（Qwen2.5-Coder-7B）改为不训练。
> 理由：复杂代码推理 → 直接调商业 API（DeepSeek-Coder / Qwen-Max-Coder）。
> 我们只需要 1.5B 知道**何时调** + **怎么把外脑结果光湖化**。
> 节省 ¥4,000-6,000 + 2-4 天训练时间 + 一份模型维护成本。

### Phase C · MP-1.5B 人格大脑（每人格一个）
- 模型: Qwen3-1.7B-Base
- 步骤:
  1. M0 → 1.7B 蒸馏（世界观下传）
...
| Phase 1 写 Kimi+DeepSeek 全参训练 | **修正**为 Qwen3-8B + 8×Qwen3-1.7B + 商业 API |
...
|---|---|
| < 5M | 路径 X · 纯 LoRA |
| 5M - 200M | **路径 Y · 1.7B 全参微调（最可能落地）** |
| 200M - 1B | 路径 Z · 8B CPT + 1.7B 蒸馏 |
...
### 8.2.5 已知初始数据点（2026-05-01）
- GPT 聊天记录已上传部分: **697.7 KB** ≈ 14 万 token
- 完整语料（GPT + Notion + GitHub）总估算: **6.5M - 37M token**
- 大概率结论：**落在路径 Y · 1.7B 全参微调**
- 但仍以真实导入为准
...
4. ✅ `factory/training/` 训练脚本骨架（Qwen3-8B CPT + DeepSpeed 配置）
