...
| 层级 | 名称 | 系统 |
|------|------|------|
| 观察层 | 零点原核频道 | 主控观察与指令入口 |
| 核心大脑 | Notion 系统 | 认知、记忆、决策、调度 |
| 执行层 | Repository 系统 | 代码执行、自动化、部署 |
...
**核心原则**：观察层不直接控制执行层。所有任务必须通过核心大脑调度。
...
├── communication-map.json ← 通信结构地图 · 自动生成
└── id-map.json            ← 统一编号体系
```
...
|----------|------|------|
| ICE | 语言主控层编号 | ICE-GL |
| TCS | 系统节点身份编号 | TCS-0002∞ |
| DEV | 人类开发任务编号 | DEV-012 |
| PER | 人格体角色编号 | PER-001 |

编号档案：`brain/id-map.json`
用途：名字 → 编号自动补全
...
目标：人格体自动理解 — 系统在哪、自己是谁、自己负责什么、当前要做什么。
