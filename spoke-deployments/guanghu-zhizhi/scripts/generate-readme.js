...
// 铸渊 · README Dashboard 总生成器（副控增强版）
// 读取各数据源 → 拼接成美观的 README.md
// 之之专属：含铸渊指令签发区（区块五）
//
// 环境变量:
//   DEV_ID       - 开发者编号 (DEV-004)
//   DEV_NAME     - 开发者名称 (之之)
//   DEV_ROLE     - 开发者角色 (零点原核语言副控·仓库人类主控)
//   PERSONA_NAME - 人格宝宝名称 (秋秋)
//   PERSONA_ID   - 人格宝宝编号 (PER-Q0001)
...
try {
  const latest = fs.readFileSync('zhuyuan-instructions/LATEST.md', 'utf8');
  const summary = latest.substring(0, 300).split('\n').slice(0, 8).join('\n');
...
console.log('✅ README Dashboard 已生成（副控增强版 · 含铸渊指令签发区）');
