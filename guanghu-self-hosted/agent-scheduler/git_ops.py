...
class GitOps:
    """Git operations manager for a single repository clone."""
    ...
    async def clone_or_pull(self) -> GitResult:
        ...
        if os.path.isdir(os.path.join(self.repo_dir, ".git")):
            ...
            result = await _run_git(["pull", "--rebase"], cwd=self.repo_dir)
            return result
        else:
            ...
            if result.success:
                # Configure author
                await _run_git(
                    ["config", "user.name", self.config.commit_author_name],
                    cwd=self.repo_dir,
                )
                await _run_git(
                    ["config", "user.email", self.config.commit_author_email],
                    cwd=self.repo_dir,
                )
            return result
    ...
    async def checkout_branch(self, branch: str, create: bool = True) -> GitResult:
        ...
        # First try to checkout existing
    ...
    async def add_files(self, paths: Optional[List[str]] = None) -> GitResult:
        """Stage files. If paths is None, stages all changes."""
        if paths:
    ...
    def write_file(self, relative_path: str, content: str) -> str:
        """Write content to a file in the repo directory.
