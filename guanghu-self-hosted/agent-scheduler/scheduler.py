...
Updated: GH-INTG-001 - supports API mode via WorkOrderAPIClient.
...
class WorkOrderDB:
    """Work order database operations (direct DB access)."""
...
# ---------------------------------------------------------------------------
# Order Source factory
# ---------------------------------------------------------------------------

def create_order_source(config: AppConfig):
    """Create the appropriate order source based on config.

    If config.api.use_api is True and api_key is set, uses WorkOrderAPIClient.
    Falls back to direct DB access via WorkOrderDB.
    If fallback_to_db is True, wraps in HybridOrderSource.

    Returns:
        (order_source, api_client_or_none, db_or_none)
    """
    db = WorkOrderDB(config.db.dsn)

    if config.api.use_api and config.api.api_key:
        ...
        if config.api.fallback_to_db:
            order_source = HybridOrderSource(api_client=api_client, db_fallback=db)
            logger.info("Order source: API + DB fallback (hybrid mode)")
        else:
            order_source = api_client
            logger.info("Order source: API only (no DB fallback)")
        return order_source, api_client, db
    else:
        logger.info("Order source: direct DB (API not configured)")
        return db, None, db
...
class AgentScheduler:
    ...
    def __init__(self, config: AppConfig):
        ...
        self._running = True
        # Order source will be set during boot
        self._order_source: Optional[Any] = None
        self._api_client: Optional[Any] = None
        self._db: Optional[WorkOrderDB] = None

    async def boot(self):
        """Initialize: load Boot Protocol, connect order source, clone repo."""
        ...
        # Step 2: Order source (API or DB)
        self._order_source, self._api_client, self._db = create_order_source(self.config)

        if self._api_client:
            await self._api_client.connect()
            healthy = await self._api_client.is_healthy()
            if healthy:
                logger.info("Work Order API is healthy")
            else:
                logger.warning("Work Order API health check failed")
    ...
    async def execute_order(self, order: Dict[str, Any]):
        ...
        order_number = order.get("order_number", order.get("order_code", ""))
        dev_content = order.get("dev_content", order.get("description", ""))
        ...
        try:
            # Step 1: Accept (claim if using API)
            if self._api_client and hasattr(self._order_source, 'claim_order'):
                try:
                    await self._order_source.claim_order(order_id)
                except Exception as exc:
                    logger.warning("Claim via API failed, updating status directly: %s", exc)
                    await self._order_source.update_order_status(order_id, "developing")
            else:
                await self._order_source.update_order_status(order_id, "developing")

            await self._order_source.write_execution_log(order_id, self.config.scheduler.agent_id, {
                "step": "accept",
            ...
            # Step 6: Self-check
            await self._order_source.update_order_status(order_id, "self_check")
            ...
            await self._order_source.update_order_status(
                order_id, "awaiting_review",
                extra_fields={
            ...
            await self._order_source.update_order_status(order_id, "error", extra_fields={
    ...
    async def poll_loop(self):
        ...
        while self._running:
            try:
                orders = await self._order_source.fetch_pending_orders(
                    self.config.scheduler.agent_id
                )
                if orders:
                    order = orders[0]
                ...
                # Periodically try to recover API if it was down
                if hasattr(self._order_source, 'recover_api'):
                    await self._order_source.recover_api()
