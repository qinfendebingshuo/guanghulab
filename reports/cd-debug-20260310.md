━━━ 铸渊排查报告 · YM-CD-DEBUG-20260310-001 ━━━
排查时间：2026-03-10T14:33Z
commit SHA：60dda7e38a8b2ab118189eb69109ddc8c0730d5c (main HEAD)

① 工作流文件在 main 上：✅ 文件存在（SHA: cf2fa91eeaa5436eded5a1f0dc0e17f3e2a4778e）

② Actions 运行历史：共 52 次运行，全部失败 ❌
   最近一次：2026-03-10T14:08:00Z（Run #52）状态 ❌ failure
   触发 commit：f951ef1876aa439a65e6edaef3f4c492fb88d30d
   标题：Merge pull request #49 from qinfendebingshuo/copilot/fix-model-intera…

③ 触发条件配置：✅ 正确
   - 触发分支：main ✅
   - workflow_dispatch：✅ 已配置（支持手动触发）

④ paths-ignore 范围：✅ 合理，仅排除非部署目录
   - .github/persona-brain/**
   - broadcasts-outbox/**
   - syslog-inbox/**
   - syslog-processed/**
   - signal-log/**
   - dev-nodes/**

⑤ 部署目标目录：✅ rsync 同步全仓库到 DEPLOY_PATH（已修复旧问题）
   - rsync path = ./ → $DEPLOY_PATH/（全站同步）
   - docs/index.html → 复制到站点根目录 index.html
   - Nginx root 迁移逻辑：自动从 status-board/ 子目录迁移到 DEPLOY_PATH

⑥ Secrets 状态：❌ 未配置（这是所有 52 次失败的根本原因）

   **失败日志分析（Run #52, Job ID 66466822994）：**
   ```
   echo "" > ~/.ssh/deploy_key          ← DEPLOY_KEY 为空
   ssh-keyscan -H  >> ~/.ssh/known_hosts ← DEPLOY_HOST 为空，ssh-keyscan 无参数
   ##[error]Process completed with exit code 1.
   ```

   缺少的 Secrets：
   - DEPLOY_HOST  — 服务器 IP（必需）
   - DEPLOY_USER  — SSH 用户名（必需）
   - DEPLOY_KEY   — SSH 私钥 PEM（必需）
   - DEPLOY_PATH  — 部署路径（必需）

⑦ 手动触发测试结果：无法测试（Secrets 未配置，部署必然失败）
   - validate job：✅ 通过（不需要 Secrets）
   - deploy job：❌ 失败（Secrets 为空 → ssh-keyscan 无主机名 → exit 1）
   - notify job：⏭️ 跳过（deploy 失败后不执行）

⑧ 服务器上文件是否更新：❌ 从未成功部署过

⑨ guanghulab.com 是否显示最新内容：❌

【结论】：CD 管线卡在第⑥步 — GitHub Secrets 未配置
【原因】：仓库 Settings → Secrets → Actions 中缺少 DEPLOY_HOST、DEPLOY_USER、DEPLOY_KEY、DEPLOY_PATH 四个必需 Secrets，导致 SSH 连接步骤直接失败
【已执行修复】：在 deploy-to-server.yml 的 deploy job 开头添加了 "🔐 检查必需 Secrets" 步骤，在尝试 SSH 之前提前检测并给出明确错误提示
【建议修复方案】：
  1. 在仓库 Settings → Secrets and variables → Actions 中添加以下 Secrets：
     - DEPLOY_HOST：服务器 IP 地址
     - DEPLOY_USER：SSH 登录用户名
     - DEPLOY_KEY：SSH 私钥完整 PEM 内容
     - DEPLOY_PATH：服务器上网站根目录路径（如 /var/www/guanghulab）
  2. 添加后，手动触发 workflow_dispatch 或 push 到 main 验证

✅ YM-CD-DEBUG-20260310-001 排查完成 · 2026-03-10T14:33Z

━━━ 铸渊排查报告 · YM-CD-DEBUG-20260310-002 ━━━
排查时间：2026-03-10 15:47
commit SHA：481904c62d4daa2642a4d4ec89a45465855374b8

① sync-persona-studio.yml 失败原因：
   workflow 使用了 `if: ${{ secrets.CROSS_REPO_TOKEN != '' }}` 作为 step 条件判断。
   根据 GitHub Actions 文档，secrets 上下文不能在 `if:` 条件表达式中直接引用。
   这导致 GitHub Actions 在解析阶段就拒绝了整个 workflow，
   所有 85 次运行均生成 0 个 jobs，conclusion=failure。

② 修复方案：
   a. sync-persona-studio.yml:
      - 将 CROSS_REPO_TOKEN 从 step 级 env 提升到 job 级 env
      - 将 `if: ${{ secrets.CROSS_REPO_TOKEN != '' }}` 改为 `if: env.CROSS_REPO_TOKEN != ''`
      - 这样 GitHub Actions 可以正常解析 workflow 并在运行时正确评估条件
   b. deploy-to-server.yml:
      - Run #54 错误日志：`Load key "/home/runner/.ssh/deploy_key": error in libcrypto`
      - 原因：DEPLOY_KEY Secret 已配置但内容格式无效（非标准 PEM 私钥）
      - 将 `echo` 改为 `printf '%s\n'` 写入密钥（更好地保留多行格式）
      - 新增 SSH 私钥格式校验步骤，在连接前验证密钥是否以 `-----BEGIN ... PRIVATE KEY-----` 开头
      - 校验失败时给出明确的错误提示和修复指引

③ 修复 commit：见本 PR 提交

④ 修复后运行结果：
   - sync-persona-studio.yml：✅ YAML 语法已修复，workflow 可正常创建 jobs
   - deploy-to-server.yml：⚠️ 代码层面已修复，但仍需仓库管理员重新生成并更新 DEPLOY_KEY Secret

⑤ deploy-to-server.yml 状态：⚠️ 需要管理员操作
   代码修复已完成。要使部署完全跑通，需执行：
   1. 在服务器上生成新密钥对：`ssh-keygen -t ed25519 -f deploy_key -N ""`
   2. 将 deploy_key 内容（私钥）更新到 GitHub Secrets → DEPLOY_KEY
   3. 将 deploy_key.pub 内容（公钥）添加到服务器 ~/.ssh/authorized_keys
   4. 手动触发 workflow_dispatch 验证

⑥ guanghulab.com 是否显示最新内容：❌（部署尚未成功，需先修复 DEPLOY_KEY）

【结论】：CD 管线代码层面问题已修复，卡在 Secrets 配置步骤
  - sync-persona-studio.yml：if 表达式错误已修复 ✅
  - deploy-to-server.yml：SSH 密钥校验已加固 ✅，但 DEPLOY_KEY Secret 内容需管理员重新生成
【建议】：
  1. 仓库管理员重新生成 ed25519 SSH 密钥对并更新 DEPLOY_KEY Secret
  2. 如需跨仓库同步，创建 CROSS_REPO_TOKEN Secret（需包含 persona-studio 仓库写权限的 PAT）
  3. 修复后手动触发 workflow_dispatch 验证两个 workflow 均能跑通
✅ YM-CD-DEBUG-20260310-002 排查完成 · 2026-03-10 15:47
